...
[RequireComponent (typeof (CameraCollider))]
public class MyStratCamera : MonoBehaviour {

	public Vector3 PanLimits = new Vector3 (100, 0, 100);
	public Vector3 MousePoint;
	public float CamDistMin = 1.0f;
	public float CamDistMax = 12.0f;
	...
	private void TurnCameraAroundPoint(float deltaX, float deltaY) {
		...
		newCamHpr.x = Mathf.Clamp(camHpr.x - deltaY, 10, 85);
	...
	private void SetTarget (Vector3 position) {
		position.x = Mathf.Clamp (position.x, 0, PanLimits.x);
		position.y = Mathf.Clamp (position.y, 0, PanLimits.y);
		position.z = Mathf.Clamp (position.z, 0, PanLimits.z);
	...
	void LateUpdate () {
		...
		if (zoomDelta != 0) {
			CamDist -= CamDist * zoomDelta;
			CamDist = Mathf.Clamp (CamDist, CamDistMin, CamDistMax);
		}
		...
		// Move the camera with arrow keys (or WASD etc. depending on the project input settings)
